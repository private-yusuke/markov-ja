...
class MarkovJa {
  ...
  makeTriplet (arr) {
    for (const array of arr) {
      ...
      for (let i = 0; i < array.length - 2; i++) {
        const triplet = array.slice(i, i + 3)
        if (!this.rootTriplet[triplet]) {
          this.rootTriplet[triplet] = 1
        } else {
          this.rootTriplet[triplet]++
        }
      }
      let triplet = [MarkovJa.BEGIN, array[0], array[1]]
      if (!this.rootTriplet[triplet]) {
        this.rootTriplet[triplet] = 1
      } else {
        this.rootTriplet[triplet] += 1
  ...
  generateSentence () {
    const morphemes = []
    const firstTriplet = this.getFirstTriplet()
    morphemes.push(firstTriplet[1])
    morphemes.push(firstTriplet[2])

    while (morphemes[morphemes.length - 1] !== MarkovJa.END) {
      const prefix1 = morphemes[morphemes.length - 2]
      const prefix2 = morphemes[morphemes.length - 1]
      const triplet = this.getTriplet(prefix1, prefix2)
      morphemes.push(triplet[2])
    }
    const result = morphemes.slice(0, morphemes.length - 1).join('')
    return result
  }
  ...
  getTriplet (prefix1, prefix2) {
    ...
    const chains = this.getChainFromDB(prefixes)
    const tmp = this.getProbableTriplet(chains)
    if (!tmp) {
      return [null, null, MarkovJa.END]
    }
    const triplet = Object.keys(tmp)[0].split(',')
  ...
  getChainFromDB (prefixes) {
    ...
    if (prefixes.length === 1) {
      for (const kelem of Object.keys(this.rootTriplet)) {
  ...
  getProbableTriplet (chains) {
    const probability = []
    const iter = chains.entries()
    for (const elem of iter) {
      for (let j = 0; j < Object.values(elem[1])[0]; j++) {
   ...
   */
  getFirstTriplet () {
    const prefixes = [MarkovJa.BEGIN]
    const chains = this.getChainFromDB(prefixes)
    const triplet = this.getProbableTriplet(chains)
    if (!triplet) {
      return [null, null, MarkovJa.END]
    }

    const triplets = Object.keys(triplet)[0].split(',')
    const res = [triplets[0], triplets[1], triplets[2]]
    return res
  }
   ...
   */
  removeTriplets (str) {
    const sentences = this.morphplogicalAnalysis(str)
